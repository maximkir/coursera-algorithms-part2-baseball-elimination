...
public class BaseballElimination {
    ...
    public boolean isEliminated(String teamName){
        validateTeamName(teamName);
        return teamEliminationStats.computeIfAbsent(teamName, t -> isTeamEliminated(t)).isEliminated;
    }
    ...
    private TeamEliminationStats isTeamEliminated(String teamName){
        ...
        for (String otherTeam : teams()){
            if (!otherTeam.equals(teamName) && wins(otherTeam) > maxTeamWins){
                return TeamEliminationStats.eliminatedStats(Collections.singletonList(otherTeam));
            }
        }
    ...
    private TeamEliminationStats isNonTriviallyEliminated(String teamName){
        ...
        int teamId = teams.get(teamName),
            checkedTeamMaxWins = wins[teamId] + remaining[teamId],
            gamesBetweenTeamsCount = gamesBetweenTeamsCount();
        ...
        for (int i = 0; i < numberOfTeams(); i++){
            ...
            fn.addEdge(new FlowEdge(virtualTeamIndex.get(i), tVertex, checkedTeamMaxWins - wins[i] > 0 ? checkedTeamMaxWins - wins[i] : 0));
        }
        ...
        if (totalLeftGames > ff.value()){
            ...
            return TeamEliminationStats.eliminatedStats(certificateOfElimination);
        }
    ...
    private static class TeamEliminationStats {
        ...
        public static TeamEliminationStats eliminatedStats(List<String> certificateOfElimination){
